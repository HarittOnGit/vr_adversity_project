...
import './components/car-controls.js';

// Memoized Scene to prevent re-renders when points/speed change
    ...
    >
      ...
      <a-entity>
        {Array.from({ length: 50 }).map((_, i) => (
          <a-entity key={i} position={`8 0 ${-i * 400}`}>
      ...
      <a-entity>
        {Array.from({ length: 25 }).map((_, i) => (
          <a-entity key={i} position={`-10 0 ${-i * 800}`}>
...
export default function App() {
  ...
  return (
    <div className="w-full h-screen relative font-sans overflow-hidden bg-black">
      {/* HUD Overlay */}
      <div className="absolute top-8 left-8 z-10 pointer-events-none space-y-4">
        ...
        <div className="bg-black/60 backdrop-blur-md border border-white/10 p-4 rounded-xl shadow-2xl">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="text-xl font-mono text-white leading-none uppercase">{highBeam ? 'High' : 'Low'}</div>
          </div>
